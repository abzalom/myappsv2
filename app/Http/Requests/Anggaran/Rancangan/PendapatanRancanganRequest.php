...
class PendapatanRancanganRequest extends FormRequest
{
    ...
    public function storerekening()
    {
        ...
        Pendapatan4ObjekRancangan::firstOrCreate([
        ...
        Pendapatan5RincianRancangan::firstOrCreate([
        ...
        return Pendapatan6SubrincianRancangan::firstOrCreate([
    ...
    {
        $data = Pendapatan7UraianRancangan::where([
            'kode_subrincian' => $subrincian->kode_subrincian,
            'tahun' => tahun(),
        ])->orderBy('kode_uraian', 'desc');
        if ($data->get()->count() == 0) {
            return Pendapatan7UraianRancangan::create([
    ...
    {
        $uraian = Pendapatan7UraianRancangan::find($this->iduraian);
        $jumlah = str_replace(',', '.', $this->jumlah);
        // return $jumlah;
    ...
    {
        $uraian = Pendapatan7UraianRancangan::find($this->id);
        $uraian->delete();
    }

    public function restoreuraian()
    {
        $uraian = Pendapatan7UraianRancangan::onlyTrashed()->find($this->id);
        $uraian->restore();
        return $uraian;
    }
}
