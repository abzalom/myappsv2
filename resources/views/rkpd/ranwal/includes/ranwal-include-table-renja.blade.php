                                    ...
                                    <td class="text-center" style="width: 5%">
                                        @can('input-renja')
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                <a href="/ranwal/rkpd/opd/{{ $opd->id }}/subkegiatan/{{ $subkegiatan->id }}/subkeluaran" type="button" class="btn btn-sm btn-primary"><i class="fa-solid fa-plus-square"></i></a>
                                                <a href="/ranwal/rkpd/opd/{{ $opd->id }}/subkegiatan/{{ $subkegiatan->id }}/edit" type="button" class="btn btn-sm btn-info"><i class="fa-solid fa-edit"></i></a>
                                                @if (!$subkegiatan->subkeluarans_count)
                                                    <form action="/ranwal/rkpd/opd/{{ $opd->id }}/subkegiatan/destroy" method="post">
                                                        @csrf
                                ...
                                @foreach ($subkegiatan->subkeluarans as $subkeluaran)
                                    <tr class="fst-italic" id="subkeltr">
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                @can('input renja')
                                                    <a href="/ranwal/rkpd/opd/{{ $opd->id }}/subkegiatan/{{ $subkegiatan->id }}/subkeluaran/{{ $subkeluaran->id }}/edit" type="button" class="btn btn-sm btn-info"><i class="fa-solid fa-edit"></i></a>
                                                    <form action="/ranwal/rkpd/opd/{{ $opd->id }}/subkegiatan/{{ $subkegiatan->id }}/subkeluaran/{{ $subkeluaran->id }}/destroy" method="post">
                                                        @csrf
                                                        <input type="hidden" name="aksi" value="delete">
