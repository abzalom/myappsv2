        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <span class="card-title">Rancangan Renja Perangkat Daerah Tahun Anggaran {{ session()->get('tahun') }}</span>
                </div>
                <div class="card-body">
                    <div class="row mb-3">
                        @role(['admin', 'bappeda'])
                            <div class="col-6">
                                <form action="/rancangan/rkpd/renja/all/upload" method="post" id="formUploadRenjaAllOpd" enctype="multipart/form-data">
                                    @csrf
                            ...
                            <tbody>
                                @foreach ($opds as $opd)
                                    <tr>
                                        <td>{{ $opd->kode_opd }}</td>
                                        <td>{{ $opd->nama_opd }}</td>
                                        <td class="text-end">{{ number_format($opd->pagurancangans_sum_jumlah, 2, ',', '.') }}</td>
                                        <td class="text-end">{{ number_format($opd->rancangansubkeluarans_sum_anggaran, 2, ',', '.') }}</td>
                                        <td class="text-end">{{ number_format($opd->pagurancangans_sum_jumlah - $opd->rancangansubkeluarans_sum_anggaran, 2, ',', '.') }}</td>
                                        <td>
                                            <a href="/rancangan/rkpd/opd/{{ $opd->id }}" class="btn btn-sm btn-primary">Renja</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
